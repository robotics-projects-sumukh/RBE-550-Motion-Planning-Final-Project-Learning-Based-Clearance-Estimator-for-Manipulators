...
INVERSE_TRANSFORM_FACTOR = 0.459463625131557

# Define the neural network model
class DistancePredictor(nn.Module):
    def __init__(self, input_size=20, hidden_layers=[1400, 1400], output_size=1): 
...
model.load_state_dict(torch.load('model/model_cnrrt_2_obs.pth', map_location=device))
print("Model loaded")


def predict_distance(model, s: ob.State, position: list[list[float]]):
    j11 = s[0][0] / (0.38615) 
    j12 = s[0][1] / (1.6056)
    j13 = s[0][2] / (1.518)
    j14 = s[0][3] / (3.14)
    ...
    obstacle_sphere_x = position[0][0]
    obstacle_sphere_y = position[0][1]
    obstacle_sphere_z = position[0][2]
    obstacle_sphere_length = obstacle_radius*2
    obstacle_sphere_breadth = obstacle_radius*2
    obstacle_sphere_height = obstacle_radius*2
    obstacle_cube_x = position[1][0]
    obstacle_cube_y = position[1][1]
    obstacle_cube_z = position[1][2]
    obstacle_cube_length = obstacle_radius*2
    obstacle_cube_breadth = obstacle_radius*2
    obstacle_cube_height = obstacle_radius*2

    # Prepare input data and transfer it to GPU
    input_data = [j11, j12, j13, j14, j15, j2, j3, j4, obstacle_sphere_x, obstacle_sphere_y, obstacle_sphere_z, obstacle_sphere_length, obstacle_sphere_breadth, obstacle_sphere_height, obstacle_cube_x, obstacle_cube_y, obstacle_cube_z, obstacle_cube_length, obstacle_cube_breadth, obstacle_cube_height]
...
class CustomValidityChecker(ob.StateValidityChecker):
    ...
    def isValid(self, state: ob.State) -> bool:
        ...
        closest_points = list(self.world.get_closest_points(False))
        return not self.world.in_collision() and min(closest_points).distance > 0.05


class CustomPathLengthObjective(ob.PathLengthOptimizationObjective):
    groups: list[GroupABC]
    def __init__(self, si: ob.SpaceInformation, world: World, set_state: SetStateFn, groups: list[GroupABC], position: list[list[float]]):
        super().__init__(si)
        self.group = groups[0]
        self.world = world
        self.set_state = set_state
        ...
        print("CustomPathLengthObjective initialized.")

    def motionCost(self, s1: ob.State, s2: ob.State) -> ob.Cost:

        self.set_state(s1, self.world)
        clearance_state1 = predict_distance(model, s1, self.position)
        cost1 = 1 / (clearance_state1 + 1e-4)
        self.set_state(s2, self.world)
        clearance_state2 = predict_distance(model, s2, self.position)
    ...
    def stateCost(self, state: ob.State) -> ob.Cost:
        ...
        clearance = predict_distance(model, state, self.position)
...
def get_motion_path_cost(path: og.PathGeometric, si: ob.SpaceInformation, sc: ValidityChecker, position: list[list[float]]) -> float:
    cost = 0
    cost_og = 0
    print("get_path_cost")
    for i in range(path.getStateCount()-1):
        ...
        clearance_state1 = predict_distance(model, s1, position)
        cost1 = 1 / (clearance_state1 + 1e-4)
        clearance_state2 = predict_distance(model, s2, position)
        cost2 = 1 / (clearance_state2 + 1e-4)
        avg = (cost1 + cost2) / 2
