...
INVERSE_TRANSFORM_FACTOR = 0.3336035936724558
# Define the neural network model
class DistancePredictor(nn.Module):
    def __init__(self, input_size=11, hidden_layers=[1400, 1400], output_size=1): 
...
model.load_state_dict(torch.load('model/model_cnrrt_table.pth', map_location=device))
print("Model loaded")


def predict_distance(model, s: ob.State, position: list):
    j11 = s[0][0] / (0.38615) 
    j12 = s[0][1] / (1.6056)
    j13 = s[0][2] / (1.518)
    j14 = s[0][3] / (3.14)
    ...
    j3 = s[2].value / (2.16)
    j4 = s[3].value / (3.14)
    obstacle_x = 0.4764751954341656
    obstacle_y = 0.8894896905893123
    obstacle_z = 0.6984669621486252

    # Prepare input data and transfer it to GPU
    input_data = [j11, j12, j13, j14, j15, j2, j3, j4, obstacle_x, obstacle_y, obstacle_z]
...
class CustomValidityChecker(ob.StateValidityChecker):
    ...
    def __init__(self, si: ob.SpaceInformation, world: World, set_state: SetStateFn, position: list):
        super().__init__(si)
        self.world = world
    ...
    def isValid(self, state: ob.State) -> bool:
        ...
        return not self.world.in_collision()


class CustomPathLengthObjective(ob.PathLengthOptimizationObjective):
    groups: list[GroupABC]
    def __init__(self, si: ob.SpaceInformation, world: World, set_state: SetStateFn, groups: list[GroupABC], position: list):
        super().__init__(si)
        self.group = groups[0]
        self.world = world
        self.set_state = set_state
...
def get_motion_path_cost(path: og.PathGeometric, si: ob.SpaceInformation, sc: ValidityChecker, position: list) -> float:
    cost = 0
    cost_og = 0
    print("get_path_cost")
...
def main(
    ...
    # Load modeled environment
        ...
        world.add_skeleton(problem_resource.environment, self_collision = False)
        position = [0.4764751954341656, 0.8894896905893123, 0.6984669621486252]
    ...
    if validity_checker_type == "basic":
        validity_checker = CustomValidityChecker(context.setup.getSpaceInformation(), world, context.set_state, position)
    ...
    context.set_validity_checker(validity_checker_original)
    # Set the PathLengthObjective as the optimization objective
    custom_path_length_objective = CustomPathLengthObjective(context.si, world, context.set_state, [request.group], position)
    ...
    if benchmark:
        ...
        while plan_num < 1:
            ...
            if context.plan(request.planning_time):
                ...
                cost, cost_og = get_motion_path_cost(path, context.si, validity_checker_original, position)
