...
# Define the inverse transform factor
...
def predict_distance(model, s: ob.State, position: list):
    ...
    j3 = s[2].value / (2.16)
    j4 = s[3].value / (3.14)
    obstacle_x = position[0]
    obstacle_y = position[1]
    obstacle_z = position[2]
    obstacle_length = 0.3
...
class CustomValidityChecker(ob.StateValidityChecker):
    ...
    def isValid(self, state: ob.State) -> bool:
        ...
        return not self.world.in_collision() and predict_distance(model, state, self.position) > 0.05
...
def main(
    ...
    # Load modeled environment
        ...
        # Output the result
        position_x = origin_xyz_values[0][0]
        position_y = origin_xyz_values[0][1]
        position_z = origin_xyz_values[0][2]
        position = [position_x, position_y, position_z]
    ...
    # Set the custom Path Length Objective.
